...
const addStory = asyncErrorWrapper(async  (req,res,next)=> {
    ...
    const {title,content, age, height, weight, price, status} = req.body 
...
const getAllStories = asyncErrorWrapper( async (req,res,next) =>{
    ...
    query =searchHelper("title, age",query,req)
...
const editStory  =asyncErrorWrapper(async(req,res,next)=>{
    const {slug } = req.params ; 
    const {title ,content, age, height, weight, price, status ,image ,previousImage } = req.body;
